...
/**
 * StandardFonts use WinAnsi encoding and throw on emoji / many Unicode symbols.
 * Replace common punctuation, then drop remaining non-Latin-1 characters.
 */
...
export async function generateBaaMarkdownPdf(input: {
  ...
  const pdfTitle = sanitizePdfText(markdownTitle || input.title);
